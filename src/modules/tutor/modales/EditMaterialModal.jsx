...
import { X, FileText, FolderTree, Layers } from "lucide-react"

export function EditMaterialModal({ material, onClose, onUpdate }) {
  const espaciosMock = ["Aula 101", "Aula 102", "Laboratorio B", "Biblioteca"]
  const tiposArchivo = ["PDF", "PPT", "DOCX", "Video", "Imagen"]

  const [formData, setFormData] = useState({
    id: material.id,
    nombre: material.nombre,
    espacio: material.espacio,
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-100 relative">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Editar Material</h2>
              <p className="text-gray-500 text-xs">Actualiza los datos del material</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <FieldInput
            ...
            name="nombre"
            placeholder="Ej: Introducción a Arquitecturas"
          ...
          <FieldSelect
            label="Espacio"
            icon={<FolderTree className="h-3.5 w-3.5 text-gray-400" />}
            name="espacio"
            value={formData.espacio}
            onChange={handleChange}
            options={espaciosMock}
          />
